...
// --- Meta Pulse: живая строка фактов из текущих данных ---
function renderMetaPulse(){const track=document.getElementById('metaPulseTrack');if(!track||!heroes.length)return;const withPro=heroes.filter(h=>Number(h.pro_pick||0)>0);const reliable=withPro.filter(h=>Number(h.pro_pick||0)>=20);const bestWr=[...reliable].sort((a,b)=>winrate(b)-winrate(a))[0];const mostPicked=[...withPro].sort((a,b)=>Number(b.pro_pick||0)-Number(a.pro_pick||0))[0];const mostBanned=[...heroes].sort((a,b)=>Number(b.pro_ban||0)-Number(a.pro_ban||0))[0];const facts=[];if(bestWr)facts.push(`📈 Лучший pro-WR: <b>${escapeHtml(bestWr.localized_name)}</b> ${winrate(bestWr).toFixed(1)}%`);if(mostPicked&&Number(mostPicked.pro_pick||0)>0)facts.push(`🔥 Чаще всего пикают: <b>${escapeHtml(mostPicked.localized_name)}</b> · ${statValue(mostPicked.pro_pick)} picks`);if(mostBanned&&Number(mostBanned.pro_ban||0)>0)facts.push(`⛔ Чаще всего банят: <b>${escapeHtml(mostBanned.localized_name)}</b> · ${statValue(mostBanned.pro_ban)} banов`);const roleCount=new Set(heroes.flatMap(h=>h.roles||[])).size;facts.push(`🗂 В базе: <b>${heroes.length}</b> героев${items.length?` · <b>${items.length}</b> предметов`:''}${roleCount?` · <b>${roleCount}</b> ролей`:''}`);if(items.length){const priciest=[...items].filter(i=>Number(i.cost||0)>0).sort((a,b)=>Number(b.cost||0)-Number(a.cost||0))[0];if(priciest)facts.push(`💰 Самый дорогой предмет: <b>${escapeHtml(priciest.dname)}</b> · ${statValue(priciest.cost)}g`);}if(facts.length<3){const pick=heroes[Math.floor(Math.random()*heroes.length)];if(pick)facts.push(`🎲 Попробуй героя: <b>${escapeHtml(pick.localized_name)}</b> · ${escapeHtml(roleText(pick))}`);}if(!facts.length)return;const html=facts.map(f=>`<span class="mp-item">${f}</span>`).join('');track.innerHTML=html+html;}
