const API="/api/dota/heroes", ITEMS_API="/api/dota/items";
...
let heroes=[],items=[],filter="all",itemFilter="all",guideFilter="all",lastFocusedEl=null,wardInterval=null,spotlightHeroId=null,spotlightTimer=null;
let myTeam={Carry:null,Mid:null,Offlane:null,Support:null,HardSupport:null},enemyTeam=[];
...
const guideData=[
 {cat:"gameplay",icon:"🎯",tag:"GAMEPLAY",title:"Как выбрать героя на матч",text:"Смотри на роль, задачи состава, тип атаки и ограничения противников.",action:"heroes"},
 {cat:"heroes",icon:"⚔",tag:"HEROES",title:"Понимание ролей",text:"Роль — это ориентир. Проверяй профиль героя и строй план игры вокруг его сильных сторон.",action:"stats"},
 {cat:"items",icon:"◈",tag:"ITEMS",title:"Как читать предметы",text:"Смотри не только на стоимость: важны характеристики, активные способности и синергия.",action:"items"},
 {cat:"gameplay",icon:"🧠",tag:"GAMEPLAY",title:"Подготовка к рейтинговой игре",text:"Проверь пул героев, ключевые предметы и план первых минут до поиска матча.",action:"tools"},
 {cat:"tools",icon:"🎲",tag:"TOOLS",title:"Случайный пик для тренировки",text:"Расширяй пул героями, которых редко берёшь в обычных матчах.",action:"tools"},
 {cat:"heroes",icon:"📊",tag:"HERO DATA",title:"Как читать статистику",text:"Pro picks показывают популярность на профессиональной сцене, а Pro win — победы среди этих матчей.",action:"stats"},
 {cat:"items",icon:"💰",tag:"ECONOMY",title:"Предметы по бюджету",text:"Фильтруй базу по стоимости и открывай предмет, чтобы увидеть связанные рекомендации.",action:"items"},
 {cat:"gameplay",icon:"🗺",tag:"MAP",title:"Контроль карты",text:"Вард — не просто предмет: это привычка планировать следующий безопасный участок карты.",action:"tools"},
 {cat:"heroes",icon:"🔗",tag:"COMPANION",title:"Герой → гайд → билд",text:"Сохраняй подходящий билд прямо из профиля героя и используй его снова.",action:"heroes"},
 {cat:"tools",icon:"🧮",tag:"MMR",title:"Планирование MMR",text:"Калькулятор показывает математический ориентир, а не гарантированный результат.",action:"tools"}
];
...
function imageUrl(h){const slug=slugForHero(h);const p=h?.img||"";if(p.startsWith("http"))return p;if(p.startsWith("/"))return `https://cdn.dota2.com${p.split("?")[0]}`;return `https://cdn.dota2.com/apps/dota2/images/dota_react/heroes/${slug}.png`;}
...
function itemImage(x){const p=x?.img||"";if(p.startsWith("http"))return p;if(p.startsWith("/"))return `https://cdn.dota2.com${p.split("?")[0]}`;return `https://cdn.dota2.com/apps/dota2/images/dota_react/items/${itemSlug(x?.name||x)}.png`;}
...
async function loadHeroes(force=false){
  ...
  try{
    ...
    const data=await d2hFirstSuccessful([
      ()=>d2hFetchJSON(`/api/dota/heroes${force?'?refresh=1':''}`),
...
async function loadItems(force=false){
  ...
  try{
    ...
    const data=await d2hFirstSuccessful([
      ()=>d2hFetchJSON(`/api/dota/items${force?'?refresh=1':''}`),
...
function favoriteIds(){try{return JSON.parse(localStorage.getItem('d2h_favorites')||'[]').map(Number)}catch{return[]}}
async function toggleFavorite(id){let f=favoriteIds();f=f.includes(Number(id))?f.filter(x=>x!==Number(id)):[...f,Number(id)];localStorage.setItem('d2h_favorites',JSON.stringify(f));renderHeroFavoriteButton(id);if(localStorage.getItem('d2h_token')){try{const r=await fetch('/api/profile/favorites',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({heroIds:f})});if(!r.ok)throw new Error('Не удалось сохранить избранное');}catch(e){alert(e.message)}}await renderProfile();}
function renderHeroFavoriteButton(id){const b=document.getElementById('favHeroBtn');if(b){const on=favoriteIds().includes(id);b.textContent=on?'♥ В избранном':'♡ В избранное';}}
function statValue(v){return Number(v||0).toLocaleString('ru-RU');}
function updateHeroUI(source){heroes=heroes.filter(Boolean);document.getElementById('heroCount').textContent=heroes.length;document.getElementById('quickHeroCount').textContent=heroes.length;document.getElementById('heroRoleCount').textContent=new Set(heroes.flatMap(h=>h.roles||[])).size||'—';document.getElementById('status').textContent=`Загружено ${heroes.length} героев · ${source}`;renderHeroSpotlight();renderHeroes();renderFeaturedHeroes();renderStats();renderDraftPicker();renderProfile();}
...
function renderGuides(){const list=guideData.filter(g=>guideFilter==='all'||g.cat===guideFilter);document.getElementById('guidesGrid').innerHTML=list.map(g=>`<article class="guide-card guide-rich-card"><div class="guide-art">${g.icon}</div><div><span>${g.tag}</span><h3>${g.title}</h3><p>${g.text}</p><button class="btn ghost guide-open" data-go="${g.action}">Открыть раздел →</button></div><b>↗</b></article>`).join('');}
function randomHero(){if(!heroes.length)return;const h=heroes[Math.floor(Math.random()*heroes.length)];document.getElementById('randomToolText').textContent=`Сегодня судьба выбрала ${h.localized_name}. ${roleText(h)}.`;openHero(h.id);}
function mmr(){const a=Number(prompt('Текущий MMR:',1000)),b=Number(prompt('Желаемый MMR:',2000));if(!Number.isFinite(a)||!Number.isFinite(b))return;const delta=Math.max(0,b-a),wins=Math.ceil(delta/25);alert(b<=a?'🏆 Ты уже достиг этой отметки!':`Ориентир: ${wins} побед по +25 MMR без учёта поражений.`);}
function wardTimer(){let n=60;clearInterval(wardInterval);document.getElementById('wardTimer').textContent=`${n} сек`;wardInterval=setInterval(()=>{n--;document.getElementById('wardTimer').textContent=n>0?`${n} сек`:'Готов';if(n<=0)clearInterval(wardInterval)},1000);}
function renderDraftPicker(){const q=(document.getElementById('draftSearch')?.value||'').toLowerCase();const list=heroes.filter(h=>(h.localized_name||'').toLowerCase().includes(q)).slice(0,36);document.getElementById('draftHeroGrid').innerHTML=list.map(h=>`<button data-draft-id="${h.id}" title="${escapeHtml(h.localized_name)}"><img src="${imageUrl(h)}" alt=""><span>${escapeHtml(h.localized_name)}</span></button>`).join('');renderDraftSlots();}
function renderDraftSlots(){const roles=Object.keys(myTeam);document.getElementById('myTeam').innerHTML=roles.map(r=>{const h=myTeam[r];return `<button class="role-slot" data-role="${r}">${h?`<img src="${imageUrl(h)}"><span>${r}<b>${escapeHtml(h.localized_name)}</b></span>`:`<span>${r}<b>+ Выбрать</b></span>`}</button>`}).join('');document.getElementById('enemyTeam').innerHTML=enemyTeam.map(h=>`<button class="enemy-slot" data-remove-enemy="${h.id}"><img src="${imageUrl(h)}"><span>${escapeHtml(h.localized_name)}</span>×</button>`).join('')+(enemyTeam.length<5?'<button class="enemy-add">+ Добавить героя</button>':'');document.querySelectorAll('.role-slot').forEach(b=>b.onclick=()=>{window.draftRole=b.dataset.role;document.getElementById('draftSearch').focus();});document.querySelectorAll('[data-remove-enemy]').forEach(b=>b.onclick=()=>{enemyTeam=enemyTeam.filter(x=>x.id!==Number(b.dataset.removeEnemy));renderDraftSlots();renderDraftResults();});renderDraftResults();}
function draftPick(h){const role=window.draftRole;if(role&&myTeam[role]===null){myTeam[role]=h;window.draftRole=null;}else if(enemyTeam.length<5&&!enemyTeam.some(x=>x.id===h.id))enemyTeam.push(h);renderDraftSlots();renderDraftResults();}
function renderDraftResults(){const el=document.getElementById('draftResults');if(!enemyTeam.length){el.innerHTML='<div class="empty">Добавь хотя бы одного героя противника.</div>';return;}const pool=heroes.filter(h=>!enemyTeam.some(e=>e.id===h.id)&&!Object.values(myTeam).filter(Boolean).some(m=>m.id===h.id));const scores=pool.map(h=>{let score=5, reasons=[];enemyTeam.forEach(e=>{if(e.primary_attr!==h.primary_attr){score+=.25;reasons.push(`давит ${e.localized_name}`)}if((h.roles||[]).includes('Disabler')){score+=.15;reasons.push('даёт контроль')}});const allies=Object.values(myTeam).filter(Boolean);allies.forEach(a=>{if(a.primary_attr!==h.primary_attr){score+=.1;}});score=Math.min(9.9,score);return {h,score,reasons:[...new Set(reasons)].slice(0,2)}}).sort((a,b)=>b.score-a.score).slice(0,3);el.innerHTML=`<h3>Лучшие варианты</h3>${scores.map((x,i)=>`<article class="draft-result"><div class="rank">${i+1}</div><img src="${imageUrl(x.h)}"><div><h4>${escapeHtml(x.h.localized_name)} <span>${x.score.toFixed(1)}/10</span></h4><p>${x.reasons.join(' · ')||'Универсальный вариант для состава.'}</p><small>Синергия: ${escapeHtml(roleText(x.h))}</small></div><button class="btn ghost" data-draft-open="${x.h.id}">Профиль</button></article>`).join('')}`;document.querySelectorAll('[data-draft-open]').forEach(b=>b.onclick=()=>openHero(Number(b.dataset.draftOpen)));}
...
function openHero(id){const h=heroes.find(x=>Number(x.id)===Number(id));if(!h)return;lastFocusedEl=document.activeElement;const a=attrInfo(h.primary_attr),stats=heroStats(h),counters=counterCandidates(h),build=heroBuild(h),fav=favoriteIds().includes(h.id);const abilities=['Основная способность','Контроль / прокаст','Усиление героя','Ультимейт'];document.getElementById('modalContent').innerHTML=`<div class="hero-detail"><div class="hero-cover"><img src="${imageUrl(h)}" alt="${escapeHtml(h.localized_name)}"><div><div class="eyebrow">HERO PROFILE</div><h2>${escapeHtml(h.localized_name)}</h2><p>${a[0]} ${a[1]} · ${escapeHtml(h.attack_type||'Тип атаки')} · ${escapeHtml(roleText(h))}</p><div class="hero-detail-actions"><button class="btn red" id="favHeroBtn">${fav?'♥ В избранном':'♡ В избранное'}</button><a class="btn ghost" target="_blank" rel="noopener" href="${officialHeroUrl(h)}">Официальная страница ↗</a></div></div></div><div class="detail-stats">${stats.map(x=>`<div><small>${x.k}</small><strong>${x.v}</strong></div>`).join('')}</div><div class="detail-section"><h3>Способности</h3><div class="ability-grid">${abilities.map((x,i)=>`<article class="ability-card"><div class="ability-art"><img src="${imageUrl(h)}" alt=""><span>${i+1}</span></div><div><b>${x}</b><p>Подробное описание и значения загружаются из актуального Dota 2 datafeed в онлайн-режиме.</p><small>Клик по профилю → актуальные данные</small></div></article>`).join('')}</div></div><div class="detail-columns"><div><h3>Контрпики</h3><div class="linked-list">${counters.map(x=>`<button data-hero-open="${x.id}"><img src="${imageUrl(x)}">${escapeHtml(x.localized_name)}<span>→</span></button>`).join('')}</div></div><div><h3>Рекомендуемый билд</h3><div class="build-list">${build.map((x,i)=>`<button data-item-by-name="${escapeHtml(x)}"><span>${i+1}</span>${escapeHtml(x)}</button>`).join('')}</div><button class="btn ghost save-build" id="saveBuildBtn">Сохранить билд</button></div></div><div class="detail-section item-popularity-section" id="heroItemPopularity"><div class="item-pop-loading">Загружаем реальные покупки предметов…</div></div><div class="detail-section"><h3>Гайды</h3><div class="guide-mini-grid">${guideData.slice(0,3).map(g=>`<article><span>${g.icon} ${g.tag}</span><b>${g.title}</b><button data-go="guides">Открыть →</button></article>`).join('')}</div></div></div>`;document.getElementById('modal').classList.add('show');document.getElementById('close').focus();document.getElementById('favHeroBtn').onclick=()=>toggleFavorite(h.id);document.getElementById('saveBuildBtn').onclick=()=>saveBuild(h,build);document.querySelectorAll('[data-hero-open]').forEach(b=>b.onclick=()=>openHero(Number(b.dataset.heroOpen)));document.querySelectorAll('[data-item-by-name]').forEach(b=>b.onclick=()=>{const term=b.dataset.itemByName.toLowerCase();const x=items.find(i=>String(i.dname).toLowerCase().includes(term.split(' ')[0]));if(x)openItem(x.name);});loadHeroItemPopularity(h.id);}
async function saveBuild(h,build){const payload={heroId:h.id,hero:h.localized_name,items:build,name:'Мой билд'};if(localStorage.getItem('d2h_token')){try{const r=await fetch('/api/profile/builds',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!r.ok){const d=await r.json().catch(()=>({}));throw new Error(d.error||'Не удалось сохранить билд');}}catch(e){alert(e.message);return}}else{const all=JSON.parse(localStorage.getItem('d2h_builds')||'[]');all.unshift(payload);localStorage.setItem('d2h_builds',JSON.stringify(all.slice(0,20)));}await renderProfile();alert(localStorage.getItem('d2h_token')?'Билд сохранён на сервере.':'Войди в аккаунт, чтобы сохранить билд на сервере.');}
...
async function renderProfile(){const fg=document.getElementById('favoritesGrid'),bg=document.getElementById('savedBuilds'),gg=document.getElementById('myGuides'),pn=document.getElementById('profileName');if(!fg&&!bg&&!gg)return;if(localStorage.getItem('d2h_token')){try{const r=await fetch('/api/profile');if(r.ok){const d=await r.json();localStorage.setItem('d2h_favorites',JSON.stringify((d.favorites||[]).map(x=>Number(x.id))));localStorage.setItem('d2h_builds',JSON.stringify(d.builds||[]));localStorage.setItem('d2h_guides',JSON.stringify(d.guides||[]));if(pn)pn.textContent=d.user?.username||'Игрок';}}catch{}}if(!localStorage.getItem('d2h_token')){if(fg)fg.innerHTML='<div class="empty">Войдите в аккаунт, чтобы использовать профиль.</div>';if(bg)bg.innerHTML='<div class="empty">Войдите в аккаунт, чтобы сохранять билды.</div>';if(gg)gg.innerHTML='<div class="empty">Войдите в аккаунт, чтобы создавать гайды.</div>';if(pn)pn.textContent='Игрок';return;}const f=favoriteIds().map(id=>heroes.find(h=>h.id===id)).filter(Boolean);if(fg)fg.innerHTML=f.length?f.map(h=>`<button data-profile-hero="${h.id}"><img src="${imageUrl(h)}"><span>${escapeHtml(h.localized_name)}</span></button>`).join(''):'<div class="empty">Пока пусто. Добавь героя в избранное.</div>';const builds=JSON.parse(localStorage.getItem('d2h_builds')||'[]');if(bg)bg.innerHTML=builds.length?builds.map(b=>`<div class="saved-row"><b>${escapeHtml(b.hero_name||b.hero||'Герой')}</b><span>${(b.items||[]).map(escapeHtml).join(' · ')}</span></div>`).join(''):'<p class="muted">Сохрани билд из Build Lab.</p>';const guides=JSON.parse(localStorage.getItem('d2h_guides')||'[]');if(gg)gg.innerHTML=guides.length?guides.map(g=>`<div class="saved-row"><b>${escapeHtml(g.title)}</b><span>${escapeHtml(g.hero_name||g.hero||'')} · ${escapeHtml(g.role||'')}</span></div>`).join(''):'<p class="muted">Создай первый пользовательский гайд.</p>';document.querySelectorAll('[data-profile-hero]').forEach(b=>b.onclick=()=>openHero(Number(b.dataset.profileHero)));}
async function createGuide(){if(!localStorage.getItem('d2h_token')){location.href='auth.html?next='+encodeURIComponent(location.href);return}const title=prompt('Название гайда:','Мой гайд');if(!title)return;const hero=prompt('Герой:','');const role=prompt('Позиция (1–5):','2')||'2';try{const r=await fetch('/api/profile/guides',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,hero,role,content:'',published:false})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Не удалось сохранить гайд');const guides=JSON.parse(localStorage.getItem('d2h_guides')||'[]');guides.unshift(d.guide);localStorage.setItem('d2h_guides',JSON.stringify(guides));await renderProfile();alert('Гайд сохранён на сервере.')}catch(e){alert(e.message)}}
...
click('randomBtn',randomHero);click('randomBtn2',randomHero);click('mmrBtn',mmr);click('wardBtn',wardTimer);click('itemSuggestBtn',()=>{if($('itemSuggestion'))$('itemSuggestion').textContent=suggestions[Math.floor(Math.random()*suggestions.length)];});click('draftBtn',()=>go('draft'));click('excuseBtn',()=>{if($('excuse'))$('excuse').textContent=excuses[Math.floor(Math.random()*excuses.length)];});
...
on('draftSearch','input',renderDraftPicker);on('draftHeroGrid','click',e=>{const b=e.target.closest('[data-draft-id]');if(b)draftPick(heroes.find(h=>h.id===Number(b.dataset.draftId)));});
click('clearDraft',()=>{myTeam={Carry:null,Mid:null,Offlane:null,Support:null,HardSupport:null};enemyTeam=[];renderDraftPicker();});
click('renameProfile',()=>{location.href='profile.html';});
click('clearProfile',()=>{if(confirm('Очистить локальный кэш профиля? Данные на сервере не удаляются.')){localStorage.removeItem('d2h_favorites');localStorage.removeItem('d2h_builds');localStorage.removeItem('d2h_guides');renderProfile();}});
click('newGuideBtn',createGuide);
