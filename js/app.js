...
async function toggleFavorite(id){let f=favoriteIds();f=f.includes(Number(id))?f.filter(x=>x!==Number(id)):[...f,Number(id)];localStorage.setItem('d2h_favorites',JSON.stringify(f));renderHeroFavoriteButton(id);await renderProfile();}
...
async function saveBuild(h,build){const payload={heroId:h.id,hero:h.localized_name,items:build,name:'Мой билд'};const all=JSON.parse(localStorage.getItem('d2h_builds')||'[]');all.unshift(payload);localStorage.setItem('d2h_builds',JSON.stringify(all.slice(0,20)));await renderProfile();alert('Билд сохранён в этом браузере.');}
...
click('renameProfile',()=>{const cur=localStorage.getItem('d2h_display_name')||'';const name=prompt('Имя профиля:',cur);if(name===null)return;if(name.trim())localStorage.setItem('d2h_display_name',name.trim());else localStorage.removeItem('d2h_display_name');renderProfile();});
