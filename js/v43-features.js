...
(function(){
  ...
  function renderGlobal(q=''){
    ...
    const sections=[['Разделы',[['Герои','heroes'],['Предметы','items'],['Статистика','stats'],['Гайды','guides'],['О сайте','about']].filter(x=>x[0].toLowerCase().includes(q))],['Герои',hs],['Предметы',is]];
