...
(function(){
  ...
  function renderGlobal(q=''){
    ...
    const sections=[['Разделы',[['Герои','heroes'],['Предметы','items'],['Статистика','stats'],['Мета','meta'],['Гайды','guides'],['Инструменты','tools'],['Конструктор билда','builds'],['Помощник матча','match-assistant'],['Помощник перед пиком','draft'],['О сайте','about']].filter(x=>x[0].toLowerCase().includes(q))],['Герои',hs],['Предметы',is]];
