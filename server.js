...
for(const rc of [redis,pub,sub])rc.on('error',e=>console.error('Redis client error:',e.message));
...
function dedupe(key,fn){if(inFlight.has(key))return inFlight.get(key);const p=(async()=>{try{return await fn()}finally{inFlight.delete(key)}})();inFlight.set(key,p);return p}
...
function clientIp(req){const h=req.headers['x-forwarded-for'];if(h){const parts=String(h).split(',').map(x=>x.trim()).filter(Boolean);if(parts.length)return parts[parts.length-1]}return req.socket.remoteAddress}
...
async function fetchJsonSafe(url){const r=await fetch(url,{headers:{Accept:'application/json'},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});if(!r.ok) throw new Error('HTTP '+r.status+' for '+url);return r.json();}
...
async function getItemsList(){
  ...
  const merged=await dedupe(cacheKey,fetchItemsList);
...
async function dotaProxy(req,res,u){
  ...
  if(u.pathname==='/api/dota/heroes'){
    try{
      const r=await fetch('https://api.opendota.com/api/heroStats',{headers:{'Accept':'application/json','User-Agent':'Dota2-Helper/1.0'},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});
...
async function getOfficialRuMap(){
  ...
  const map=await dedupe(cacheKey,async()=>{
    const r=await fetch(VDF_RU_URL,{headers:{Accept:'text/plain'},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});
    if(!r.ok) throw new Error('VDF HTTP '+r.status);
    return parseVdfTokens(await r.text());
  });
...
async function fetchOfficialItem(itemId){
  const r=await fetch(`https://www.dota2.com/datafeed/itemdata?language=english&item_id=${encodeURIComponent(itemId)}`,{headers:{Accept:'application/json'},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});
...
async function getItemDetail(itemId){
  ...
  if(!raw){
    raw=await dedupe(cacheKey,()=>fetchOfficialItem(itemId));
...
async function getHeroItemPopularity(heroId){
  ...
  try{const cached=await redis.get(cacheKey);if(cached) return JSON.parse(cached);}catch{}
  const data=await dedupe(cacheKey,async()=>{
    const r=await fetch(`https://api.opendota.com/api/heroes/${encodeURIComponent(heroId)}/itemPopularity`,{headers:{Accept:'application/json'},signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)});
...
async function viewersCount(channel){try{return Number(await redis.sCard(`presence:${channel}`))||0}catch(e){console.error('viewersCount degraded (Redis unavailable), returning 0 for',channel,':',e.message);return 0}}
async function initRedis(){try{await Promise.all([redis.connect(),pub.connect(),sub.connect()]);await sub.pSubscribe('chat:*',message=>{const channel=message.slice(message.indexOf(':')+1);let data;try{data=JSON.parse(message)}catch{return}broadcast(channel,data)});await sub.pSubscribe('presence:*',async message=>{const channel=message.slice(message.indexOf(':')+1);let data;try{data=JSON.parse(message)}catch{return}broadcast(channel,data)})}catch(e){console.error('Redis init failed, running in degraded mode:',e.message)}}
...
async function handleWsMessage(ws,m){const ch=ws.channel;if(m.type==='chat'){const c=await channelBySlug(ch),claim=ws.claim;if(claim&&c&&await one('SELECT 1 FROM bans WHERE channel_id=$1 AND user_id=$2',[c.id,claim.id]))return;const text=String(m.text||'').trim().slice(0,300);if(!text)return;const msg={id:crypto.randomUUID(),user:String(claim?.username||'Гость').slice(0,32),text,ts:now()};await redis.lPush(`chat:${ch}`,JSON.stringify(msg));await redis.lTrim(`chat:${ch}`,0,299);await pub.publish(`chat:${ch}`,JSON.stringify({type:'chat',message:msg}))}
 if(m.type==='delete'){const c=await channelBySlug(ch),claim=ws.claim;if(c&&await canModerate(claim,c.id)){await redis.eval("local items=redis.call('LRANGE',KEYS[1],0,-1) for i,v in ipairs(items) do local ok,obj=pcall(cjson.decode,v) if ok and obj.id==ARGV[1] then redis.call('LREM',KEYS[1],1,v) return 1 end end return 0",{keys:[`chat:${ch}`],arguments:[String(m.messageId)]});await pub.publish(`chat:${ch}`,JSON.stringify({type:'delete',messageId:m.messageId}))}}
